...
export default async function collectionUpdatedHandler({
  ...
  const webhookUrl = `${FRONTEND_URL}/api/revalidate/collections`;
  console.log(`Product collection ${data.id} updated, calling ${webhookUrl}`);
  ...
  try {
    ...
    console.error("Failed to notify storefront about collection update", error);
  }
}
